...
def test_create_receipt(client: TestClient, logged_user: dict[str, str]) -> None:
    ...
    data = {
        "user_id": 2,
        "sale_items": [{"product_id": 2, "quantity": 1}],
        "payment_type": "card",
        "payment_amount": 620000,
    }
    response = client.post(
        f"http://{settings.DOMAIN}:8000/receipts/", headers=logged_user, json=data
    )
...
def test_cant_create_receipt_for_other_user(client: TestClient) -> None:
...
def test_get_receipts(client: TestClient, logged_user: dict[str, str]) -> None:
    """Test getting all receipts for a user.

    The user has id 2 and has receipts in the database."""
    response = client.get(
        f"http://{settings.DOMAIN}:8000/receipts/", headers=logged_user
    )
    assert response.status_code == 200
...
def test_get_receipt_by_id(client: TestClient, logged_user: dict[str, str]) -> None:
    """Test getting a receipt by id.

    The receipt has id 2 and belongs to user with id 2."""
    response = client.get(
        f"http://{settings.DOMAIN}:8000/receipts/2", headers=logged_user
    )
    assert response.status_code == 200
    receipt = response.json()
    assert receipt["id"] == 2
...
def test_cant_get_receipt_by_id_for_other_user(client: TestClient) -> None:
...
def test_get_receipt_text(client: TestClient, logged_user: dict[str, str]) -> None:
    ...
    expected_end = "========================================"
    response = client.get(
        f"http://{settings.DOMAIN}:8000/receipts/2/text", headers=logged_user
    )
    assert response.status_code == 200
...
def test_can_get_receipt_text_for_other_user(client: TestClient) -> None:
...
def test_get_receipts_by_user(client: TestClient, logged_user: dict[str, str]) -> None:
    """Test getting all receipts for a user.
...
def test_get_public_receipt(client: TestClient) -> None:
